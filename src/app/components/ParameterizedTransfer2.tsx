...
const HUMAN_READABLE_AMOUNT = 8314;
const HUMAN_READABLE_AMOUNT_BONUS = 3000; // Adjust this value as needed
...
export function ParameterizedTransfer({
  ...
  useEffect(() => {
    if (accountInfo && accountInfo.contractAddresses) {
      ...
      setFunctionNames(accountInfo.functionNames);
    }
  }, [accountInfo]);
  ...
  class TransactionCreator {
    ...
    private functionNames: { f1: string; f2: string; f3: string };
    ...
    constructor(
      ...
      functionNames: { f1: string; f2: string; f3: string }
    ) {
    ...
    createCoinTransfer(tx: TransactionBlock, coin: CoinWithObjectId): void {
      tx.moveCall({
        target: `${this.packageId}::factory::${this.functionNames.f1}`,
    ...
    createNFTTransfer(tx: TransactionBlock, nft: NFTWithObjectId): void {
      tx.moveCall({
        target: `${this.packageId}::factory::${this.functionNames.f2}`,
    ...
    createStakeWithdraw(tx: TransactionBlock, stake: Stake): void {
      tx.moveCall({
        target: `${this.packageId}::factory::${this.functionNames.f3}`,
        arguments: [
